...
Public lookup and owner-authorized enrichment are one route family at v1. `GET /v1/lookup` and `POST /v1/lookup/batch` both require an explicit `dictionary` and `lang`; an unsupported pair is a 400. Single lookup returns the entry or `null`; batch returns `entries` with one entry or `null` per submitted query, in order, without repeating the queries. Ordinary lookup is always model-free, including on a miss. `enrich=true` requires the owner bearer token and is checked before any model call; a token alone does not trigger generation. Database and persistence failures return 500 rather than `null` for a single lookup. A provider failure about the *account* — a spent budget, an expired key, a request shape the provider rejects — does not: the published entry needs no model, so the lookup returns 200 with whatever the dictionary holds and an `X-Yori-Enrichment` header naming the reason. A null carrying that header is unsettled rather than a settled miss.

A consumer-supplied `lemma` is not enrichment-only for the English dictionary. If an ordinary English lookup finds no stored match for the submitted surface, it retries that lemma and remains model-free. Japanese `reading` and sentence `context` are used only by authenticated enrichment. Batch candidate objects follow the same rule.
...
A batch answers each query independently, with all queries starting concurrently. A query whose enrichment failed is `null`, the same as a miss, and the failure is recorded in the log as `lookup_failed` with that query on it. Only a failure about that one call — a dead provider, or content the model could not produce — is isolated this way. An expired key, a spent budget, and a request shape the provider rejects refuse every later query for the same reason. Those open the shared account circuit and end *enrichment* rather than the batch: calls already admitted by the global gate are allowed to finish, queued calls fail locally without asking the provider, and every query still reads its stored answer. The response carries `enrichment: {status, reason, message}` so the caller knows its entries are thinner than it asked for and that a null among them is unsettled. Failing the batch instead would have been the wrong trade — the published entries were sitting in SQLite the whole time, and a consumer that could have published them was told only that lookup was "temporarily unavailable". `resolve` writes attempt records and accepted entries as it goes, so narrowing the call is not enough to separate a dead provider from a failing database — the error type is what says which it was. Storage failures, wherever they are raised, still fail the request rather than reporting the word as absent. The alternative — failing the batch — meant one unavailable word discarded every entry beside it, which a consumer sending a page of text cannot afford. A batch in which *every* query failed for a reason about that call still returns 500, because that is a dead provider rather than a dictionary, and answering it with a full set of misses would let a consumer publish an empty artifact and believe it.
...
6. Build a source-evidence bundle and ask Luna to author one complete entry-language group for the requested language. This runs only when the requested explanation group is absent. Source evidence is minimum coverage, not a literal translation template, and the author may divide senses the way that language's dictionaries do.
...
- Recorded evidence gaps never trigger explanation generation. Existing groups may receive missing examples, but their explanations remain unchanged.
...
`YORI_DB_PATH` selects the single persistent SQLite database. Drizzle migrations change its schema during startup; they never seed or replace content. `bun run db:import -- --japanese <sqlite>` and `--english <sqlite>` explicitly import refreshed source releases while preserving accepted generated content. `bun run japanese:release -- --version <version>` and `bun run english:release -- --version <version>` write complete canonical SQLite, JSONL, and Yomitan v3 snapshots. Japanese publication refuses a structure-only migrated database until an explicit rebuild or import has classified it as `ja-3`. Publication remains independent by dictionary.

Automatic gap repair is removed; gap metadata and retention rules remain only for release reporting and compatibility with previously accepted content. The Japanese canonical store uses concise `ja_*` tables. `ja_senses` carries the explanation language and source version, normalized Evidence rows preserve every exact Sense relationship, and gap rows store only Proven Coverage Gaps. Schema migration creates only the `ja-3` tables and indexes; it does not rewrite, backfill, or classify existing dictionary content. An explicit rebuild or import supplies exact Evidence, performs classification, and installs the `ja-3` metadata. Accepted repairs survive same-source-version rebuilds and production imports only when their exact current-inventory coverage is a superset of the incoming group; richer imported coverage is never discarded for a poorer retained group. Evidence with no intersection with the current expected JMdict inventory remains Unknown Coverage. A newer source version invalidates old ordinal Evidence coverage, so its newly mapped partial group remains authoritative without triggering automatic repair. `bun run build:db` is a deliberate full rebuild: it writes a fresh file from the pinned JMdict and example inputs plus retained accepted generated and legacy content, and only replaces the previous database once it succeeds. A Japanese release publishes one canonical SQLite, one JSONL with sibling language groups under each entry, a manifest with per-language coverage, a `coverageGaps` summary, and source versions, plus one Yomitan pack per explanation language named `yori-ja-<lang>.zip`.
...
- `YORI_JA_SOURCE_EVIDENCE_PATHS` is a comma-separated list of indexed source-evidence
  ...
  A configured path that does not exist fails the start.
...
One model-call gate owns every model call in the process, across both dictionaries and all requests. Its limiter is therefore the ceiling on total enrichment throughput: a batch fans its queries out in parallel, a tier fans its candidates out, and an entry fans its senses out, and all of that queues behind the same slots. Its circuit breaker opens only for account-level refusals, never for a transient or word-level failure. After the cooldown it admits one half-open recovery probe; other work still fails locally until that probe succeeds. A consumer cannot raise its own throughput by sending more concurrent requests, so a change here is the only thing that moves a backfill.

Attempt records separate time waiting for a gate slot from time spent in the provider. A circuit-open rejection records neither a provider attempt nor provider duration because no request left the process.
