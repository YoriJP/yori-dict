...
The earlier example enrichment interface exposed persistence details and only operated after lookup found an existing entry. Adding separate entry, eligibility, source-discovery, and example modules would push orchestration into every caller and create several shallow modules.
